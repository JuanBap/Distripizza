...
# Crear un lock para asegurar la consistencia en la concurrencia
lock = threading.Lock()
...
def servidor():
    ...
    while True:
        ...
        if user_rep_socket.poll(1000):  # Tiempo de espera para solicitudes de usuario
            ...
            # Registrar el tiempo en que expira el timeout para esta solicitud (1 segundo real = 1 minuto simulado)
            user_id = solicitud.split()[1]
            solicitudes_timeout[user_id] = time.time() + (15 * 60 / 60)  # Timeout de 15 minutos en tiempo simulado
            ...
            if len(taxis) > 0:
                if user_is_still_waiting(solicitud, solicitudes_timeout):
                    ...
                    try:
                        ...
                        # Intentar reasignar el taxi
                        print("Intentando reasignar el servicio a otro taxi...")
                        taxi_seleccionado = seleccionar_taxi(taxis)
                        if taxi_seleccionado:
                            print(f"Nuevo taxi asignado: {taxi_seleccionado}")
                            # Realiza la conexión y reintento de asignación con el nuevo taxi
                            taxi_req_socket.connect(f"tcp://localhost:556{taxi_seleccionado}")
                            taxi_req_socket.send_string("Servicio asignado")
                            respuesta = taxi_req_socket.recv_string()
                            print(f"Respuesta del nuevo taxi {taxi_seleccionado}: {respuesta}")
                    finally:
